...
(qb25) cmdb@QuantBio-25 week1 %  bedtools intersect -a nhek-active.bed -b nhek-repress.bed | wc 
#   0       0       0
(qb25) cmdb@QuantBio-25 week1 %  bedtools intersect -a nhlf-active.bed -b nhlf-repress.bed | wc 
#   0       0       0

(qb25) cmdb@QuantBio-25 week1 %  bedtools intersect -a nhek-active.bed -b nhlf-active.bed | wc
#   12174   97392  781739
...
# the first commands output 12174 entries and 2405 from the second command 

#Do these two numbers add up to the original number of lines in nhek-active.bed?
#No, because there can be multiple entries from B per entry of A if there is an overlap between A and B
...
# for -f command- the nhek-active chromatin state is enclosed by the nhlf-active chromatin state, completely contained, the -F is the 
#opposite which is the nhlf-active chromatin state enclosed by the nhek-active chromatin state and the -f -F is when both
# are equal length. 
...
(qb25) cmdb@QuantBio-25 week1 %  bedtools intersect -a nhek-active.bed -b nhlf-active.bed |head
#chr1	19923013	19924213	1_Active_Promoter	0	.	19922613	19924613
# across all nine conditions, all the chromatin state are active 

#Active in NHEK, Repressed in NHLF
(qb25) cmdb@QuantBio-25 week1 %  bedtools intersect -a nhek-active.bed -b nhlf-repress.bed | head
...
(qb25) cmdb@QuantBio-25 week1 %  bedtools intersect -a nhek-repress.bed -b nhlf-repress.bed | head 
