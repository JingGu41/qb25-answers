...
#fixed the issue in the PCA ploting 
corrected_dataset <- valid_matrix
corrected_dataset[,12:13] <- valid_matrix[,13:12]
corrected_dataset
sorted_matrix <- corrected_dataset[order(row_sd, decreasing= TRUE),] #get from googling 
...
#PCA_plotting_df <- PCA_plotting_df[,c""]
#df <- df[, c("SampleID", "Tissue", "Replicate", "GeneA", "GeneB")]

#labeling obtained from google
PCA_plotting_df <- PCA_plotting_df %>%
...
```{r}
combined_df = sorted_matrix[,seq(1, 21, 3)]
combined_df= combined_df + sorted_matrix[,seq(2, 21, 3)]
combined_df = combined_df + sorted_matrix[,seq(3, 21, 3)]
...
set.seed(42)
kmeans_results <- kmeans(scale(filtered_combined), centers = 12, nstart = 100)

cluster_label <- kmeans_results$cluster
ordering <- order(cluster_label)
data_ordered <- filtered_combined[ordering, ]
label_ordered <- cluster_label[ordering]

png(file="heatmap_plot.png")
heatmap_plot <-heatmap(as.matrix(data_ordered), Rowv=NA, Colv=NA,
        RowSideColors=RColorBrewer::brewer.pal(12, "Paired")[label_ordered], scale='none')
dev.off()

#saving get from google
