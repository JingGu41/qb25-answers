...
ggplot(data = dt, aes(x= `Adipose - Subcutaneous`)) + 
  geom_histogram(breaks = seq(0, 100, 2), position ="stack") +
  labs(x= "genes", y= "expression in TPM")

# answer here
#the plot is skewed towards left which means lots of genes are tightedly regulate, not spevific to liver
```
...
```{r q3}
dt_subset <- dt %>% select("Name","Heart - Left Ventricle", "Liver", "Description")
ggplot(data = dt_subset, aes(x= dt_subset$Liver+1, y=dt_subset$`Heart - Left Ventricle`+1)) +
  geom_point(size=1, alpha=0.2)+
  geom_abline(slope=1, lty= "dashed", color="red")+
  scale_x_log10()+
  scale_y_log10()+
   labs(x= "expression in liver", y= "expression in heart")
  #theme
# answer here
```
...
```{r q4}
is_mitochondrial <- str_starts(dt$Description, pattern= "MT" )
dt<- dt%>% mutate(is_mitochondrial)
#print(dt)

ggplot(data = dt, aes(x= dt$Liver+1, y=dt_subset$`Heart - Left Ventricle`+1,colour=dt_subset$is_mitochondrial)) +
  geom_point(size=1, alpha=0.2)+
  geom_abline(slope=1, lty= "dashed", color="red")+
  scale_x_log10()+
  scale_y_log10()+
...
ggplot(data = dt_long, aes(x= TPM,scales = "free_y"))+
  geom_histogram(breaks = seq(0, 100, 2), position ="stack")+
  labs(x= "tissue", y="expression")+
    facet_grid(. ~ Tissue)
